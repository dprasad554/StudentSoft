...
public class TeacherAttendanceManagement extends AppCompatActivity implements View.OnClickListener, OnResponseListner {
  ...
  /*  @BindView(R.id.calendarView)
    ...
    public void onResponse(Object response, WebServices.ApiType apiType, boolean isSucces) {
        if (apiType == WebServices.ApiType.teacherbyid) {
            if (!isSucces) {
                ...
                teacherAttendance =  (TeacherByID) response;

                if (!isSucces || teacherAttendance == null) {
                    SnackBar.makeText(this, getResources().getString(R.string.something_wrong), SnackBar.LENGTH_SHORT).show();
                } else {
                    for (int i = 0; i < teacherAttendance.getResult().getMessage().getAttendance().size(); i++) {
                        if (teacherAttendance.getResult().getMessage().getAttendance().get(i).getType().equals("present")) {

                            try {
                                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                                Date gmt = (Date) formatter.parse(teacherAttendance.getResult().getMessage().getAttendance().get(i).getDate());
                        ...
                        } else if (teacherAttendance.getResult().getMessage().getAttendance().get(i).getType().equals("leave")) {
                            try {
                                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                                Date gmt = (Date) formatter.parse(teacherAttendance.getResult().getMessage().getAttendance().get(i).getDate());
                                long millisecondsSinceEpoch0 = gmt.getTime();
                        ...
                        }else if (teacherAttendance.getResult().getMessage().getAttendance().get(i).getType().equals("absent")) {
                            try {
                                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                                Date gmt = (Date) formatter.parse(teacherAttendance.getResult().getMessage().getAttendance().get(i).getDate());
                                long millisecondsSinceEpoch0 = gmt.getTime();
    ...
    private void initializdeletePopup(String date) {
        ...
        for (int i = 0; i < teacherAttendance.getResult().getMessage().getAttendance() .size(); i++) {
            String Reason = teacherAttendance.getResult().getMessage().getAttendance().get(i).getType();
            System.out.println("Reason::::" + Reason);
            if (date.equals(teacherAttendance.getResult().getMessage().getAttendance().get(i).getDate())) {
