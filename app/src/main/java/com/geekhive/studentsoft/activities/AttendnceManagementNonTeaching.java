...
public class AttendnceManagementNonTeaching extends AppCompatActivity implements View.OnClickListener, OnResponseListner {
    ...
    private void CallService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).Getnonteachingstaffbyid(WebServices.SM_Services,
                    WebServices.ApiType.getnonteachingstapbyid,Prefs.getPrefRefId(this),Prefs.getPrefUserAuthenticationkey(this));
        } else {
    ...
    public void onResponse(Object response, WebServices.ApiType apiType, boolean isSucces) {
        if (apiType == WebServices.ApiType.getnonteachingstapbyid) {
            if (!isSucces) {
                ...
                if (!isSucces || nonTeachingStaffById.getResult().getMessage() == null) {
                    SnackBar.makeText(this, getResources().getString(R.string.something_wrong), SnackBar.LENGTH_SHORT).show();
                } else {
                    if (nonTeachingStaffById.getResult().getMessage() != null) {
                        if (nonTeachingStaffById.getResult().getMessage().getAttendance() != null) {
                            if (nonTeachingStaffById.getResult().getMessage().getAttendance().size() != 0) {
                                for (int i = 0; i < nonTeachingStaffById.getResult().getMessage().getAttendance().size(); i++) {
                                    if (nonTeachingStaffById.getResult().getMessage().getAttendance().get(i).getType().equals("present")) {
                                        List<Event> eventList = new ArrayList<>();
                                        try {
                                            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                                            Date gmt = (Date) formatter.parse(nonTeachingStaffById.getResult().getMessage().getAttendance().get(i).getDate());
                                    ...
                                    } else if (nonTeachingStaffById.getResult().getMessage().getAttendance().get(i).getType().equals("absent")) {
                                        if (currentDateandTime.equals(nonTeachingStaffById.getResult().getMessage().getAttendance().get(i).getDate())) {
    ...
    private void initializdeletePopup(String date) {
        ...
        for (int i = 0; i < nonTeachingStaffById.getResult().getMessage().getAttendance().size(); i++) {
            String Reason = nonTeachingStaffById.getResult().getMessage().getAttendance().get(i).getType();
            System.out.println("Reason::::" + Reason);
            if (date.equals(nonTeachingStaffById.getResult().getMessage().getAttendance().get(i).getDate())) {
