...
public class StudentHomeFragment extends Fragment implements View.OnClickListener, OnResponseListner {
    ...
    private void CallService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).GetStudentbyid(WebServices.SM_Services,
                    WebServices.ApiType.getstudentbyid, Prefs.getPrefRefId(getActivity()),Prefs.getPrefUserAuthenticationkey(getActivity()));
        }
        return;
    }
        ...
        if (apiType == WebServices.ApiType.getstudentbyid) {
            ...
            if (!isSucces) {
                ...
                if (!isSucces || getStudentTimetable == null ) {
                    SnackBar.makeText(getActivity(), getActivity().getResources().getString(R.string.something_wrong), SnackBar.LENGTH_SHORT).show();
                }if (!isSucces || getStudentTimetable.getResult() == null ) {
                    SnackBar.makeText(getActivity(), getActivity().getResources().getString(R.string.something_wrong), SnackBar.LENGTH_SHORT).show();
